    ...
    <div className="animate-fade-in">
      ...
      <section className="relative h-56 md:h-72 overflow-hidden bg-charcoal text-white">
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 h-full flex flex-col justify-center items-center text-center">
          ...
          <p className="mt-3 text-base md:text-lg text-gray-100 max-w-3xl">Real lives changed — read firsthand accounts from our programs and beneficiaries across education, healthcare, and livelihoods.</p>
        </div>
      </section>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <section className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {storyContents.map((s, i) => (
            <article key={i} className="bg-white dark:bg-gray-800 rounded-lg p-6 shadow-sm border border-gray-100 dark:border-gray-700">
              <div className="mb-4 w-full h-44 bg-cover bg-center rounded" style={{ backgroundImage: `url(${s.image})` }} />
              <h3 className="text-xl font-semibold mb-2 text-gray-900 dark:text-white">Community success story #{i + 1}</h3>
              <p className="text-sm text-gray-700 dark:text-gray-200 leading-relaxed">{s.text}</p>
            </article>
          ))}
        </section>
      </div>
